...
<div class="container">
    ...
    <form action="{{ route('chart_of_accounts.update', $account) }}" method="POST" class="mb-4">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="name" id="name" value="{{ $account->name }}" class="form-control" required>
        </div>
    ...
    <form action="{{ route('chart_of_accounts.destroy', $account) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this account?');">
        @csrf
        @method('DELETE')
